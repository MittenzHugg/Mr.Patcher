...
bk_rom_manager::bk_rom_manager(n64_rom* inROM) {
	...
	//bootloader asm
	asm_code[bootloader] = new bk_file(0x1000, 0x4BE0, bk_Rom->buffer.begin(), bk_file_types::ASM, false, 0x400);
	asm_code[bootloader]->decomp();

	//Rare Devs are artful dicks.
	uint32_t crc1;
	uint32_t crc2;
	asm_code[bootloader]->get_checksums(&crc1, &crc2);
	...
	asm_code[basicLibs] = new bk_file(tmp_rom_offset, tmp_rom_size, bk_Rom->buffer.begin(), bk_file_types::ASM, true, tmp_ram_offset);
	...
	asm_vars[basicLibs] = new bk_file(tmp_rom_offset, tmp_rom_size, bk_Rom->buffer.begin(), bk_file_types::ASM_VAR, true, tmp_ram_offset);
	asm_vars[basicLibs]->decomp();
	...
	tmp_rom_size -= tmp_rom_offset;
	asm_code[engine] = new bk_file(tmp_rom_offset, tmp_rom_size, bk_Rom->buffer.begin(), bk_file_types::ASM, true, tmp_ram_offset);
	...
	asm_vars[engine] = new bk_file(tmp_rom_offset, tmp_rom_size, bk_Rom->buffer.begin(), bk_file_types::ASM_VAR, true, tmp_ram_offset);
	...
	for (uint32_t i = bk_asm_file_e::sm; i < bk_asm_file_e::expansion_pak; i++) {
		uint32_t upper = level_upper[i - bk_asm_file_e::sm];
		...
		tmp_rom_size -= tmp_rom_offset;
		asm_code[i] = new bk_file(tmp_rom_offset, tmp_rom_size, bk_Rom->buffer.begin(), bk_file_types::ASM, true);
		asm_code[i]->decomp();
		...
		asm_vars[i] = new bk_file(tmp_rom_offset, tmp_rom_size, bk_Rom->buffer.begin(), bk_file_types::ASM_VAR, true);
	...
	tmp_rom_size = 0;
	asm_code[expansion_pak] = new bk_file(tmp_rom_offset, tmp_rom_size, bk_Rom->buffer.begin(), bk_file_types::ASM, true, tmp_ram_offset);
	asm_code[expansion_pak]->comp_size = 0x400000;
	asm_vars[expansion_pak] = nullptr;
	asm_code[bootloader]->clear_buffers();
	//rom_dynamic_files
	//dyn_file_cnt = n64_get_word(bk_Rom->_buffer + 0x5E90);
	dyn_file_cnt = bk_Rom->buffer.get<uint32_t>(0x5E90);
	dyn_file_table = bk_Rom->buffer.begin() + 0x5E98;
...
void bk_rom_manager::export_rom(char* dir) {
	...
	for (int i = bk_asm_file_e::basicLibs; i < bk_asm_file_e::expansion_pak; i++) {
		strcpy(ofp, odir);
		strcat(ofp, "/");
...
void bk_rom_manager::set_asm_combo_options(QComboBox* cbox) {
	for (int i = bk_asm_file_e::sm; i < bk_asm_file_e::expansion_pak; i++) {
...
void bk_rom_manager::applyPatches(patchObj* patch_list) {
	std::vector<patchObj*> pVec;
	int patch_cnt = patch_list->count();
	pVec.reserve(patch_list->count());
	for (int i = 0; i < patch_cnt; i++) {
		pVec.push_back(patch_list->get(i + 1));
	}
	...
	for (int i = 0; i < patch_cnt; i++) {
		...
		patchable_files[5] = asm_code[bk_asm_file_e::sm + cur_lvl];
		patchable_files[6] = asm_vars[bk_asm_file_e::sm + cur_lvl];
	...
	if (asm_code[basicLibs]->modified) {
		...
		bk_Rom->buffer.set(crc1, rom_off);
		bk_Rom->buffer.set(crc2, rom_off + 0x04);
	}
	...
	if (asm_code[expansion_pak]->modified) {
		...
		n64_span boot_edit = asm_code[bootloader]->decompress();
		boot_edit.set((u32)0, 0xF8);

		//////set jump location
		boot_edit.set((u8)0x40, 0x127);
		boot_edit.set((u16)0x0000, 0x12A);
...
FILE* bk_rom_manager::packROM(void) {
	...
	fwrite(bk_Rom->buffer.begin(), sizeof(uint8_t), bk_Rom->buffer.size(), tmp_rom);
	
	//step through all asm files;
	for (int i = bk_asm_file_e::bootloader; i <= bk_asm_file_e::expansion_pak; i++) {
		if (asm_code[i] != nullptr) {
			if (asm_code[i]->modified) {
				//overwrite ROM
				if (i != bk_asm_file_e::expansion_pak) {
					asm_code[i]->comp();
				}
				else {
...
uint32_t bk_rom_manager::getAddressFromLI(n64_file* section, uint32_t upper, uint32_t lower) {
